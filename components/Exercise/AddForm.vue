...
const state = ref<TablesInsert<'exercise'>>({
  'level': 1,
  'is_active': true,
...
const schema = z.object({
  ...
  type: z.enum(ConvertEnumToArray('ExerciseTypeEnum'))
...
const insertExercise = async (event: FormSubmitEvent<Schema>) => {
  ...
  const {data,error} = await supabase.from('exercise').insert(formState).select('title').single()
  if(error){
